...
Configured ChatGPT browser results can include
`browserRun.attachmentUiReceipt` (schema
`auracall.browser_attachment_ui_receipt.v1`). It records the exact paths sent
through the browser upload path, whether upload completion was observed,
...
Each of the two bounded exact-opener attempts on a new project page now waits
inside the page for at most 15 seconds for the attachment control to mount. The
current opener is identified by its exact accessible label, `Add files and
more`, while the legacy `#composer-plus-btn` identity remains an explicit
compatibility alternative. The flow still requires visibility, a center
hit-test, a trusted pointer event, the exact local-file menu action, and one
unrestricted `Attach files` chooser before upload.
...
| `-f, --file <paths...>` | Attach local files/dirs (globs + `!` excludes). On ChatGPT's current workbench, AuraCall verifies the exact `Add photos & files` button and unique unrestricted input labeled `Attach files`; `Add library files` is a separate provider-library drawer. The legacy described row and fixed `#upload-files` input remain supported exactly. |
...
| `--browser-model-strategy <select\|current\|ignore>` | Control ChatGPT model selection in browser mode (current keeps the active model; ignore skips the picker). Chat selection binds the exact current model control and fails closed instead of treating a project-chat actions menu as the picker. |
...
```bash
# Run from any directory inside this checkout.
AURACALL_REPO_ROOT="$(git rev-parse --show-toplevel)"
