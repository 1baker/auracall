...
- Closed current ChatGPT attachment-opener repair:
  ...
  The current exact opener/menu/chooser contract was unchanged. Personal commit
  `c43e7839` used a shared trusted-pointer helper, but the sole traced guard
  `document-c43e7839-current-attachment-trusted-r1` reached that helper with
  only 145 ms of broker command budget and failed before upload or submission.
  It preserved its complete round-one Markdown/DOCX/PDF provenance but produced
  no verdict, score, review record, final-answer link, attachment receipt, or
  learning sync. The allowed provider-free rework in personal commit
  `630d1247` keeps exact hit-testing and trusted CDP input while removing inner
  transport timeouts and the synthetic fallback. It is published and installed
  with exact compiled parity; AuraCall is healthy as PID `2470517`, retained
  Chrome stayed PID `1829010`, and the guard's exact task tab is closed. No
  successful live end-to-end review is claimed and the terminal guard was not
  replayed.
...
- Closed current ChatGPT model-picker trigger repair:
  ...
  competing chat-actions trigger. Its one traced guard opened the correct menu,
  then failed closed before upload or prompt submission because `Current Pro`
  did not match the checked `Latest` option in the current flattened menu.
...
- Closed ChatGPT attachment UI receipt:
  ...
  The installed retained-browser response now records confirmed upload and
  sent-user-turn UI separately from transport intent. Provider-consumed bytes
  and downstream ModelLabs promotion remain separate gates.
...
- Closed accepted durable response-batch cancellation:
  ...
  Current state: authorized HTTP and MCP batch controls now compose the durable
  host-owned child cancellation boundary, preserve terminal results, refuse
  foreign lease takeover, and report every partial outcome. Complete local
  gates and exact-SHA Ubuntu/macOS/Windows CI pass.
...
- Closed accepted Grok semantic model selector execution:
  ...
  unknown selectors fail before browser launch, and unproved picker selection
  stops submission. Discovery readiness and the complete cross-platform CI
  matrix agree; Gemini remains explicitly deferred.
...
- Closed accepted current user path portability repair:
  ...
  audit rejects future concrete Unix, macOS, or Windows user roots while
  preserving explicit placeholders and system-shared roots.
...
- Closed accepted browser UI CDP fixture ratchet:
  ...
  eliminating warnings and suppressions without changing protocol contracts.
...
- Closed accepted production-source zero-warning lint baseline:
  ...
  on any source warning before reporting remaining test/smoke warning debt.
...
- Closed accepted broker-authoritative restart recovery:
  ...
  closed rather than entering legacy Chrome discovery. Installed response
  `resp_5ff8161469f64a61bf12107c2616ad15` survived a forced API-process restart,
  completed on the same target with exact output `RESTART_RECOVERY_PASS`, and
  released its lease. Agent-browser retained one ready browser and one valid
  tab.
...
- Closed accepted broker-first agent-browser authority:
  ...
  prerequisite for broker reuse. Provider-free gates are green, and installed
