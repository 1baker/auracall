...
describe('semantic model selectors', () => {
  it('keeps the locally enabled Gemini and Grok selectors available', () => {
    expect(SEMANTIC_MODEL_SELECTORS.filter((entry) => entry.service !== 'chatgpt').every(
      (entry) => entry.executionReady,
    )).toBe(true);
  });
