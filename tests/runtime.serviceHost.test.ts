...
describe("runtime service host", () => {
	...
	it("projects runner topology readiness without mutating runner records", async () => {
		const homeDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "auracall-runtime-service-host-topology-"),
		);
		cleanup.push(homeDir);
		setAuracallHomeDirOverrideForTest(homeDir);
	...
	it("executes browser-backed maxRuns candidates concurrently", async () => {
		...
		await control.createRun(
			createBrowserDirectBundle("run_host_browser_1", "2026-04-08T15:00:00.000Z"),
		);
		await control.createRun(
			createBrowserDirectBundle("run_host_browser_2", "2026-04-08T15:00:01.000Z"),
		);
	...
	it("loads only active run statuses for an untargeted drain", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createDirectBundle("run_host_active_filter", "2026-04-08T15:00:00.000Z"),
		);
		...
		await host.drainRunsOnce({
			...
			updatedSince: "2026-04-08T14:45:00.000Z",
			maxRuns: 1,
		});
	...
	it("honors the execution gate before acquiring a run lease", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createDirectBundle("run_host_execution_gate", "2026-04-08T15:00:00.000Z"),
		);
		const host = createExecutionServiceHost({
			control,
	...
	it("honors a configured execution gate for targeted host drains", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createDirectBundle("run_host_default_execution_gate", "2026-04-08T15:00:00.000Z"),
		);
		const host = createExecutionServiceHost({
			control,
	...
	it("uses the configured live runner id as the lease owner when claiming work", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_host_runner_owner", "2026-04-08T15:00:00.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
	...
	it("keeps host claiming scoped to the configured runner even when another eligible runner is fresher", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_host_runner_scoped_claim", "2026-04-08T15:00:00.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
	...
	it("refreshes runner-owned lease heartbeat during delayed host execution", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_host_runner_heartbeat", "2026-04-08T15:00:00.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
	...
	it("refuses to claim runnable work when the configured runner owner is unavailable", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_host_runner_missing", "2026-04-08T15:00:00.000Z"),
		);
	...
	it("preserves a requested local action when step policy requires approval and no callback is injected", async () => {
		...
		const control = createExecutionRuntimeControl();
		const bundle = createDirectBundle(
			"run_host_approval_required_action",
			"2026-04-08T15:04:30.000Z",
		);
		bundle.steps[0] = {
		...
		expect(result.drained[0]?.record?.bundle.sharedState.structuredOutputs).toContainEqual({
			...
			value: {
				...
				items: [
					{
						requestId:
							"run_host_approval_required_action:action:run_host_approval_required_action:step:1:1",
						kind: "shell",
						status: "requested",
						summary: "Queue one bounded node command for operator approval",
						command: process.execPath,
	...
	it("resolves a requested local action through the host control seam", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createRequestedLocalActionBundle("run_host_local_action_control", "2026-04-11T18:00:00.000Z"),
		);
	...
	it("resolves a requested team-run local action through the same host control seam", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createRequestedLocalActionBundle(
				"run_host_team_local_action_control",
				"2026-04-11T18:06:00.000Z",
				"team-run",
			),
		);
	...
	it("rejects resolving a local action request that is already resolved", async () => {
		...
		const control = createExecutionRuntimeControl();
		const bundle = createRequestedLocalActionBundle(
			"run_host_local_action_resolved",
			"2026-04-11T18:10:00.000Z",
		);
	...
	it("rejects resuming a run without a paused human-escalation step", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createDirectBundle("run_host_resume_not_paused", "2026-04-11T19:15:00.000Z"),
		);
	...
	it("allows a resumed team run to be reclaimed by a different compatible active runner", async () => {
		...
		expect(
			storedRecord?.bundle.leases.map((lease) => ({
	...
	it("keeps current eligible runner ownership after local-action resolution and human resume", async () => {
		...
		const resumed = await host.resumeHumanEscalation(
			"run_host_team_local_action_resume_other_runner",
			{
	...
	it("surfaces claim-owner-unavailable when targeted drain cannot safely claim a team run", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		const bundle = createDirectBundle(
			"run_host_team_targeted_drain_claim_blocked",
			"2026-04-12T22:25:00.000Z",
		);
	...
	it("repairs locally reclaimable expired leases through runner reconciliation before reclaiming work", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_reclaim_stale_runner", "2026-04-08T15:00:00.000Z"),
		);
	...
	it("reclaims an expired lease when the owning runner is still active", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_active_runner_lease", "2026-04-08T15:00:00.000Z"),
		);
	...
	it("prioritizes runnable work ahead of older recoverable stranded work under a cap", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createRunningWithoutLeaseBundle("run_stranded_old", "2026-04-08T15:00:00.000Z"),
		);
	...
	it("orders runnable work by execution priority and preserves FIFO ties", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createDirectBundle("run_priority_old_normal", "2026-04-08T15:00:00.000Z"),
		);
		await control.createRun(
			createDirectBundle("run_priority_old_high", "2026-04-08T15:01:00.000Z"),
		);
		await control.createRun(
			createDirectBundle("run_priority_new_high", "2026-04-08T15:02:00.000Z"),
		);
		const host = createExecutionServiceHost({
			control,
	...
	it("keeps priority subordinate to execution gates without acquiring a lease", async () => {
		...
		const host = createExecutionServiceHost({
			...
			executionGate: async () => ({
				allowed: false,
				reason: "capacity gate remains authoritative",
			}),
		});
	...
	it("keeps oldest-first ordering within the recoverable-stranded class", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createRunningWithoutLeaseBundle("run_stranded_oldest", "2026-04-08T15:00:00.000Z"),
		);
		await control.createRun(
			createRunningWithoutLeaseBundle("run_stranded_newest", "2026-04-08T15:01:00.000Z"),
		);
	...
	it("reserves one slot for recoverable stranded work when both actionable classes are present", async () => {
		...
		await control.createRun(
			createRunningWithoutLeaseBundle("run_stranded_reserved", "2026-04-08T15:02:00.000Z"),
		);
	...
	it("prioritizes runnable work ahead of recoverable stranded work under a cap", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createRunningWithoutLeaseBundle("run_recoverable_stranded_first", "2026-04-08T15:00:00.000Z"),
		);
	...
	it("recovers stranded running work without an active lease by rewinding to runnable", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createRunningWithoutLeaseBundle("run_stranded", "2026-04-08T15:04:00.000Z"),
		);
	...
	it("retries stranded recovery after one persist revision race", async () => {
		...
		await baseControl.createRun(
			createRunningWithoutLeaseBundle("run_stranded_retry", "2026-04-08T15:04:00.000Z"),
		);
	...
	it("summarizes reclaimable, busy, stranded, and idle recovery states", async () => {
		...
		await control.createRun(
			createRunningWithoutLeaseBundle("run_stranded", "2026-04-08T15:01:00.000Z"),
		);
	...
	it("suppresses task-run-spec linkage on recovery detail for direct runs", async () => {
		...
		const control = createExecutionRuntimeControl();
		const bundle = createDirectBundle(
			"run_detail_direct_task_spec_hidden",
	...
	it("preserves bounded local claim status map when the configured runner record is unavailable", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createDirectBundle("run_local_claim_missing_runner", "2026-04-08T15:00:00.000Z"),
		);
	...
	it("projects local claims from listed records with one runner read and no per-run reinspections", async () => {
		const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "auracall-runtime-service-host-"));
		cleanup.push(homeDir);
		setAuracallHomeDirOverrideForTest(homeDir);
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_local_claim_single_pass_a", "2026-04-08T15:00:00.000Z"),
		);
		await control.createRun(
			createDirectBundle("run_local_claim_single_pass_b", "2026-04-08T15:00:01.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
				id: "runner:single-pass",
				hostId: "host:http-responses:127.0.0.1:8080",
				startedAt: "2026-04-08T14:59:00.000Z",
				lastHeartbeatAt: "2026-04-08T15:00:30.000Z",
				expiresAt: "2026-04-08T15:10:00.000Z",
		...
		const baseInspectRun = control.inspectRun.bind(control);
		const baseReadRunner = runnersControl.readRunner.bind(runnersControl);
		let inspectRunCalls = 0;
		let readRunnerCalls = 0;
		control.inspectRun = async (runId) => {
			inspectRunCalls += 1;
			return baseInspectRun(runId);
		};
		runnersControl.readRunner = async (runnerId) => {
			readRunnerCalls += 1;
			return baseReadRunner(runnerId);
		};

		const summary = await createExecutionServiceHost({
			control,
			runnersControl,
			runnerId: "runner:single-pass",
			ownerId: "host:test",
			now: () => "2026-04-08T15:05:00.000Z",
		}).summarizeLocalClaimState({ sourceKind: "direct" });

		expect(summary?.selectedRunIds.sort()).toEqual([
			"run_local_claim_single_pass_a",
			"run_local_claim_single_pass_b",
		]);
	...
	it("blocks local claim when configured service account affinity is missing from the runner", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_local_claim_account_blocked", "2026-04-08T15:00:00.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
	...
	it("selects local claim when configured service account affinity matches the runner", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_local_claim_account_selected", "2026-04-08T15:00:00.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
	...
	it("claims a scheduler-authorized local runnable run without draining it", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_scheduler_local_claim", "2026-04-08T15:00:00.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
	...
	it("drains a scheduler-claimed run through the existing local-owned lease", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_scheduler_claim_then_drain", "2026-04-08T15:00:00.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
				id: "runner:scheduler-local",
	...
	it("keeps targeted drain blocked by a foreign active lease", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_foreign_active_lease_drain", "2026-04-08T15:00:00.000Z"),
		);
		await control.acquireLease({
	...
	it("reassigns an expired stale-owner lease to the scheduler-selected local runner", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_scheduler_reassign_stale", "2026-04-08T15:00:00.000Z"),
		);
		await control.acquireLease({
		...
		expect(stored?.bundle.leases.find((lease) => lease.id === "lease:stale-owner")?.status).toBe(
			"expired",
		);
		expect(stored?.bundle.leases.find((lease) => lease.status === "active")?.ownerId).toBe(
			"runner:scheduler-local",
		);
	});
	...
	it("rejects scheduler claim when authority selects a non-local runner", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_scheduler_non_local", "2026-04-08T15:00:00.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
	...
	it("rejects scheduler claim when a fresh active lease exists", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_scheduler_fresh_lease", "2026-04-08T15:00:00.000Z"),
		);
		await control.acquireLease({
	...
	it("returns conflict when scheduler claim loses the persist revision check", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await baseControl.createRun(
			createDirectBundle("run_scheduler_revision_conflict", "2026-04-08T15:00:00.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
				id: "runner:scheduler-local",
		...
		expect(
			(await baseControl.readRun("run_scheduler_revision_conflict"))?.bundle.leases,
		).toHaveLength(0);
	});
	...
	it("preserves configured service account mismatch details on targeted drain skip", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_targeted_drain_account_blocked", "2026-04-08T15:00:00.000Z"),
		);
		await runnersControl.registerRunner({
			runner: createExecutionRunnerRecord({
								...
								items: [
									{
										...
										objective:
											"Stored consumed transfer should win over planned recovery fallback.",
										requestedOutputCount: 5,
										inputArtifactCount: 2,
									},
								],
	...
	it("classifies fresh active lease health for a live runner-owned execution", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_detail_fresh_lease", "2026-04-08T15:00:00.000Z"),
		);
		await control.acquireLease({
	...
	it("classifies suspiciously idle active lease health when no runner activity follows lease acquisition", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_detail_idle_lease", "2026-04-08T15:00:00.000Z"),
		);
		await control.acquireLease({
	...
	it("repairs only stale-heartbeat leases that are already locally reclaimable", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_detail_repairable_stale", "2026-04-08T15:00:00.000Z"),
		);
		await control.acquireLease({
	...
	it("repairs an expired active-runner lease when the runner has moved on", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_detail_active_moved_on", "2026-04-08T15:00:00.000Z"),
		);
		await control.acquireLease({
	...
	it("force-repairs an expired stale-heartbeat lease still claimed by an active runner", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_detail_force_repair", "2026-04-08T15:00:00.000Z"),
		);
		await control.acquireLease({
	...
	it("keeps suspiciously idle leases read-only for operator repair", async () => {
		...
		const runnersControl = createExecutionRunnerControl();
		await control.createRun(
			createDirectBundle("run_detail_idle_repair", "2026-04-08T15:00:00.000Z"),
		);
		await control.acquireLease({
	...
	it("cancels a planned run before it has an active lease", async () => {
		...
		const result = await host.cancelOwnedRun(
			"run_cancel_planned",
			"operator cancelled stale planned work",
		);
	...
	it("counts a reclaimed stale lease only once across repeated execution passes", async () => {
		...
		const control = createExecutionRuntimeControl();
		await control.createRun(
			createTwoStepBundle("run_multi_stale_reclaim", "2026-04-08T15:00:00.000Z"),
		);
		await control.acquireLease({
