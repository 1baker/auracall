...
describe("run archive service", () => {
	...
	test("coalesces unchanged archive-index reads and refreshes after an external change", async () => {
		...
		const first = createArchiveItemFixture({ id: "response:cache-one", responseId: "cache-one" });
		const second = createArchiveItemFixture({ id: "response:cache-two", responseId: "cache-two" });
		await writeRunArchiveIndex([first]);

		const readFile = vi.spyOn(fs, "readFile");
		await Promise.all(Array.from({ length: 20 }, () => readRunArchiveIndex()));
		...
		readFile.mockClear();
		const refreshed = await Promise.all(Array.from({ length: 20 }, () => readRunArchiveIndex()));
		expect(refreshed[0]?.items.map((item) => item.id)).toEqual([
			"response:cache-one",
			"response:cache-two",
		]);
	...
	test("coalesces archive file-metadata enrichment for repeated status-style reads", async () => {
		const homeDir = await mkdtemp(path.join(os.tmpdir(), "auracall-run-archive-metadata-cache-"));
		setAuracallHomeDirOverrideForTest(homeDir);
		const assetPath = path.join(homeDir, "large-asset.bin");
		await writeFile(assetPath, Buffer.alloc(1024 * 1024, 7));
		await writeRunArchiveIndex([
			createArchiveItemFixture({
				id: "generated-artifact:metadata-cache",
				kind: "generated_artifact",
				localPath: assetPath,
				fileName: "large-asset.bin",
			}),
		]);
		let observedAt = new Date("2026-08-25T00:00:00.000Z");
		const service = createRunArchiveService({ now: () => observedAt });
		await service.listItems({ kind: "generated_artifact" });

		const readFile = vi.spyOn(fs, "readFile");
		await Promise.all(
			Array.from({ length: 20 }, () => service.listItems({ kind: "generated_artifact" })),
		);
		expect(
			readFile.mock.calls.filter(([filePath]) => String(filePath) === assetPath).length,
		...
		expect(readFile.mock.calls.filter(([filePath]) => String(filePath) === assetPath)).toHaveLength(
			0,
		);

		observedAt = new Date("2026-08-25T00:01:00.000Z");
		readFile.mockClear();
		await service.listItems({ kind: "generated_artifact" });
		expect(readFile.mock.calls.filter(([filePath]) => String(filePath) === assetPath)).toHaveLength(
			0,
		...
		await writeFile(assetPath, Buffer.alloc(1024 * 1024 + 1, 9));
		observedAt = new Date("2026-08-25T00:02:00.000Z");
		readFile.mockClear();
		await service.listItems({ kind: "generated_artifact" });
		expect(readFile.mock.calls.filter(([filePath]) => String(filePath) === assetPath)).toHaveLength(
			1,
	...
	test("hydrates only items that can match stable list filters", async () => {
		const inaccessiblePath = await mkdtemp(
			path.join(os.tmpdir(), "auracall-run-archive-filter-scope-"),
		);
	...
	test("projects existing runtime, batch, media, upload, artifact, and provider conversation records", async () => {
		...
		expect(archive.items.map((item) => item.id)).toEqual(
		...
		expect(item?.item.links.catalogItem).toContain(
			"/v1/account-mirrors/catalog/items/conv_archive_1",
		);
		...
		expect(projectOnly.items.map((entry) => entry.id)).toContain(
			"provider-conversation:resp_archive_1:chatgpt:conv_archive_1",
		);

		const generatedArtifact = await service.readItem(
			"generated-artifact:resp_archive_1:feedback_json",
		);
		...
		await expect(
			service.readItem("generated-artifact:media_archive_1:image_1"),
		).resolves.toMatchObject({
			item: {
		...
		await expect(
			service.readAsset(`upload:resp_archive_1:resp_archive_1:step:1:upload_packet`),
		).resolves.toMatchObject({
		...
		await expect(
			service.listItems({ kind: "evidence", query: "grading-review" }),
		).resolves.toMatchObject({
			metrics: {
				total: 1,
			},
	...
	test("projects media-generation conversation identity from timeline events", async () => {
		...
		await expect(
			service.readItem("generated-artifact:media_timeline_1:image_1"),
		).resolves.toMatchObject({
			item: {
				providerConversationId: "gemini_timeline_conv_1",
				providerConversationUrl: "https://gemini.google.com/app/gemini_timeline_conv_1",
		...
		await expect(
			service.readItem("provider-conversation:media_timeline_1:gemini:gemini_timeline_conv_1"),
		).resolves.toMatchObject({
			item: {
	...
	test("refreshes indexed upload and generated artifact file metadata on archive reads", async () => {
		...
		const materializedOnlyId =
			"generated-artifact:resp_file_refresh:artifact_2:download:sandbox:/mnt/data/materialized-only.json";
		...
		const cachedAttachmentId =
			"generated-artifact:resp_file_refresh:user-message:download:sandbox:/mnt/data/cached-attachment.json";
		...
		const skippedAttachmentId =
			"generated-artifact:resp_file_refresh:user-message:download:sandbox:/mnt/data/skipped-attachment.json";
		...
		const refreshedUpload = uploadItems.items.find(
			(item) => item.id === "upload:resp_file_refresh:step:upload_1",
		);
		...
		await expect(
			service.readItem("generated-artifact:resp_file_refresh:artifact_1"),
		).resolves.toMatchObject({
			item: {
				fileAvailable: true,
				cacheKey: expect.stringMatching(/^sha256:/),
		...
		const persistedUpload = persistedIndex?.items.find(
			(item) => item.id === "upload:resp_file_refresh:step:upload_1",
		);
