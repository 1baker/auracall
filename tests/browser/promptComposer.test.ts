...
describe("promptComposer", () => {
  ...
  test('waits for a delayed ProseMirror readback before rejecting a large prompt', async () => {
	...
	test("waits for the original insertion before attempting a second composer write", async () => {
		const prompt = "Review the delayed composer packet.";
		const runtime = {
			evaluate: vi.fn()
				.mockResolvedValueOnce({ result: { value: { ready: true, composer: true } } })
				.mockResolvedValueOnce({ result: { value: { focused: true } } })
				.mockResolvedValueOnce({ result: { value: { cleared: true } } })
				.mockResolvedValueOnce({ result: { value: {
					editorText: "", fallbackValue: "", editorUserText: "", targetUserText: "",
				} } })
				.mockResolvedValueOnce({ result: { value: {
					editorText: prompt, fallbackValue: "", editorUserText: prompt,
				} } })
				.mockResolvedValueOnce({ result: { value: {
					editorText: prompt, fallbackValue: "", editorUserText: prompt,
					targetText: prompt, targetUserText: prompt,
				} } })
				.mockResolvedValueOnce({ result: { value: { ready: true } } })
				.mockResolvedValueOnce({ result: { value: "clicked" } })
				.mockResolvedValueOnce({ result: { value: {
					turnsCount: 1, lastMatched: true, lastExactMatched: true,
					lastExtraTextRecognized: true, hasNewTurn: true,
					composerCleared: true, inConversation: true,
				} } }),
		};
		const input = {
			insertText: vi.fn().mockResolvedValue(undefined),
		...
		await expect(submitPrompt(
			{ runtime: runtime as never, input: input as never, baselineTurns: 0, beforeSend },
			prompt,
