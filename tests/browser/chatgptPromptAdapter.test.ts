...
const promptActionMocks = vi.hoisted(() => ({
	...
	ensureRequiredChatgptProIntelligence: vi.fn(async () => undefined),
...
vi.mock("../../src/browser/actions/thinkingTime.js", async (importOriginal) => ({
	...
	ensureRequiredChatgptProIntelligence:
		promptActionMocks.ensureRequiredChatgptProIntelligence,
}));
...
describe("ChatGPT provider prompt adapter", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		promptActionMocks.ensureRequiredChatgptProIntelligence.mockResolvedValue(undefined);
	});
	...
	test("authorizes before submitting and projects a prompt-submitted result", async () => {
		...
		promptActionMocks.ensureRequiredChatgptProIntelligence.mockImplementation(async () => {
			events.push("pro-guard");
		});
		...
		expect(events.indexOf("composer-ready")).toBeLessThan(events.indexOf("pro-guard"));
		expect(events.indexOf("pro-guard")).toBeLessThan(events.indexOf("submit"));
		expect(onProof).toHaveBeenCalledTimes(1);
		expect(promptActionMocks.ensureModelSelection).not.toHaveBeenCalled();
		expect(promptActionMocks.ensureRequiredChatgptProIntelligence).toHaveBeenCalledWith(
			Runtime,
			expect.any(Function),
		);
