...
import type { ExecutionRunRecordStore, ExecutionRunStoredRecord } from "../src/runtime/store.js";
import { createTeamRunBundle } from "../src/teams/model.js";

function createBundle(runId = "team_run_control", createdAt = "2026-04-08T00:00:00.000Z") {
...
describe("runtime control module", () => {
	...
	it("lists stored records in one pass without rereading every returned run", async () => {
		const bundle = createBundle("team_run_single_pass");
		const record: ExecutionRunStoredRecord = {
			runId: bundle.run.id,
			revision: 1,
		...
		const store: ExecutionRunRecordStore = {
			async ensureStorage() {},
			async writeBundle() {
			...
			async listBundles() {
				throw new Error("listRuns must not list bundles and reread records");
			},
			async listRecords() {
				listRecordCalls += 1;
				return [record];
		...
		const listed = await createExecutionRuntimeControl(store).listRuns({ limit: 1 });

		expect(listed).toEqual([record]);
		expect(listRecordCalls).toBe(1);
	...
	it("filters browser authority before applying the recent-run limit", async () => {
		...
		await control.createRun(createBundle("team_run_newer_unreported", "2026-04-08T00:02:00.000Z"));
