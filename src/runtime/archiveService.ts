...
import { createExecutionResponseForStoredRecord } from "./responsesService.js";
import {
...
export interface RunArchiveService {
	...
	): Promise<RunArchiveHistoryMaterializationUpsertResult>;
...
const FILE_METADATA_CACHE_TTL_MS = 30_000;
const fileChecksumCache = new Map<
	string,
	{ mtimeMs: number; size: number; checksumSha256: string }
>();
const fileChecksumReads = new Map<string, Promise<string>>();
...
export function createRunArchiveService(deps: RunArchiveServiceDeps = {}): RunArchiveService {
	...
	const fileMetadataCache = new Map<string, { refreshedAtMs: number; items: RunArchiveItem[] }>();
	...
	async function readIndexedItems(
		...
		if (index) {
			...
			const cacheKey = createHash("sha256")
				...
				.update("\0")
				.update(scopedItems.map((item) => item.id).join("\0"))
				.digest("hex");
			const observedAtMs = now().getTime();
			const cached = fileMetadataCache.get(cacheKey);
			if (cached && observedAtMs - cached.refreshedAtMs < FILE_METADATA_CACHE_TTL_MS) {
				return cached.items;
			}
			const existingRefresh = fileMetadataRefreshes.get(cacheKey);
			...
			const refresh = refreshIndexedFileMetadata(scopedItems, {
				...
				.then((items) => {
					...
					if (fileMetadataCache.size > 64) {
						const oldestKey = fileMetadataCache.keys().next().value;
						if (oldestKey) fileMetadataCache.delete(oldestKey);
					}
					return items;
				})
				.finally(() => {
					fileMetadataRefreshes.delete(cacheKey);
				});
			fileMetadataRefreshes.set(cacheKey, refresh);
			return refresh;
		}
	...
	return {
		...
		async readAsset(id) {
			...
			return {
				...
				mimeType:
					item.mimeType ?? inferArchiveAssetMimeType(item.fileName ?? item.title ?? item.localPath),
				size: stat.size,
			};
		},
		async lookupAsset(request) {
			...
			const canonicalItem =
				limited.find((item) => item.fileAvailable === true && Boolean(item.localPath)) ??
				limited[0] ??
				null;
			const duplicateCacheKeys = Array.from(
				new Set(
					limited
		...
		async upsertMediaGenerationItems(mediaGenerationId) {
			...
			const index = await upsertArchiveItems({
				...
				items,
				removeExisting: (item) =>
					item.source === "media_generation" && item.mediaGenerationId === mediaGenerationId,
			});
			return toBackfillResult(index, timestamp);
		},
		async upsertHistoryMaterializationItems(input) {
			const timestamp = now().toISOString();
			const items = await enrichFileMetadata(
				buildHistoryMaterializationArchiveItems(input, timestamp),
			);
...
function toBackfillResult(
	index: RunArchiveIndexRecord,
	generatedAt: string,
): RunArchiveBackfillResult {
	return {
		object: "run_archive_backfill",
		generatedAt,
		index: {
...
export function createRunArchiveHistoryItemStore(
	...
	return {
		...
		async upsertItems(items) {
			...
			const nextItems = [...current.filter((item) => !incomingIds.has(item.id)), ...items].sort(
				(left, right) =>
...
function isStoredRunArchiveItem(value: unknown): value is RunArchiveItem {
	...
	return (
		value.object === "run_archive_item" &&
		typeof value.id === "string" &&
		isRunArchiveItemKind(value.kind)
	);
}
...
async function buildRunArchiveItems(
	...
	for (const record of records) {
		...
		const firstStep =
			record.bundle.steps.slice().sort((left, right) => left.order - right.order)[0] ?? null;
		const runtimeState =
			response.metadata?.executionSummary?.runtimeDiagnosticsSummary?.runtimeState ?? null;
		const base = createBaseRunItem(record, firstStep, runtimeState);
		const batch = readBatchMetadata(record);
		const responseKind: RunArchiveItemKind =
			record.bundle.run.sourceKind === "team-run" ? "team_run" : "response";
		items.push({
			...base,
			...
			metadata: {
				...
				requestedOutputCount:
					response.metadata?.executionSummary?.requestedOutputSummary?.total ?? 0,
			},
			links: {
		...
		for (const artifact of response.output.filter(isArtifactOutput)) {
			items.push({
				...
				artifactId: artifact.id,
				fileName:
					readMetadataString(artifact.metadata, ["fileName", "name"]) ?? artifact.title ?? null,
...
function buildMediaArchiveItems(records: MediaGenerationStoredRecord[]): RunArchiveItem[] {
	...
	for (const record of records) {
		...
		const base: RunArchiveItem = {
			...
			boundIdentityKey: readRecordString(response.metadata, [
				"boundIdentityKey",
				"identityKey",
				"serviceAccountId",
			]),
	...
	for (const [index, asset] of input.assets.entries()) {
		...
		items.push({
			...
			metadata: {
				...(file.metadata ?? {}),
				providerFileId:
					typeof file.metadata?.providerFileId === "string" &&
					file.metadata.providerFileId.trim().length > 0
...
function buildHistoryMaterializationArchiveItemId(
	...
	const prefix =
		asset.kind === "file"
			...
			: asset.kind === "media"
				? "history-media"
				: "history-generated-artifact";
	const identity =
		input.boundIdentityKey ?? input.runtimeProfile ?? input.browserProfile ?? "unknown-identity";
	const file = asset.file;
	return [
		prefix,
		input.provider,
		...
		sanitizeHistoryArchiveIdSegment(
			asset.artifactId ?? file.id ?? file.name ?? `asset-${index + 1}`,
		),
	].join(":");
}
...
function resolveMediaGenerationConversation(response: MediaGenerationStoredRecord["response"]): {
	id: string | null;
	url: string | null;
} {
	const metadataUrl = readRecordString(response.metadata, [
		"tabUrl",
		"conversationUrl",
		"providerConversationUrl",
		"url",
	]);
	const metadataId = readRecordString(response.metadata, [
		"conversationId",
		"conversation_id",
	...
	for (const event of timeline) {
		...
		timelineUrl =
			readRecordString(details, ["tabUrl", "conversationUrl", "providerConversationUrl", "url"]) ??
			timelineUrl;
		timelineId =
			readRecordString(details, ["conversationId", "conversation_id", "providerConversationId"]) ??
			timelineId;
	}
...
function createBaseRunItem(
	...
	const providerConversationId = readRecordString(browserRun, [
		"conversationId",
		"conversation_id",
	]);
	const providerConversationUrl = readRecordString(browserRun, [
		"tabUrl",
		"conversationUrl",
		"url",
	]);
	return {
		...
		runtimeProfile:
			firstStep?.runtimeProfileId ??
			readRecordString(record.bundle.run.initialInputs, ["runtimeProfile"]),
		browserProfile: firstStep?.browserProfileId ?? null,
		projectId: readRecordString(browserRun, ["projectId", "project"]),
		boundIdentityKey: readRecordString(browserRun, [
			"boundIdentityKey",
			"identityKey",
			"serviceAccountId",
		]),
		...
		teamId:
			record.bundle.run.sourceKind === "team-run"
				? record.bundle.run.sourceId
				: readRecordString(record.bundle.run.initialInputs, ["team"]),
		responseId: record.runId,
		batchId: null,
		batchIndex: null,
		mediaGenerationId: null,
...
function buildEvidenceArchiveItem(record: RunArchiveEvidenceRecord): RunArchiveItem {
	return {
		...
		links: {
			...(record.responseId
				? { response: `/v1/responses/${encodeURIComponent(record.responseId)}` }
				: {}),
			...(record.batchId
				? { batch: `/v1/response-batches/${encodeURIComponent(record.batchId)}` }
				: {}),
...
function listProviderConversationRefs(record: ExecutionRunStoredRecord): Array<{
	...
	for (const step of record.bundle.steps) {
		...
		const boundIdentityKey = readRecordString(browserRun, [
			"boundIdentityKey",
			"identityKey",
			"serviceAccountId",
		]);
...
function matchesStableRequest(
	...
	if (request.status && item.status !== request.status && item.runtimeState !== request.status)
		return false;
	return true;
}
...
function matchesRequest(
	...
	if (typeof request.fileAvailable === "boolean" && item.fileAvailable !== request.fileAvailable)
		return false;
...
function normalizeAssetLookupRequest(
	request: RunArchiveAssetLookupRequest,
): RunArchiveAssetLookupResult["query"] {
	const query = {
...
function matchesAssetLookupRequest(
	item: RunArchiveItem,
	query: RunArchiveAssetLookupResult["query"],
): boolean {
	...
	return Promise.all(
		items.map(async (item) => {
			const cachedConversationEvidence = item.localPath
				? null
				: await findCachedConversationAttachmentEvidence(item);
			...
			const unavailableEvidence: Record<string, unknown> | null =
				localFile.unavailableErrorCode
					? buildUnavailableLocalFileEvidence(
							discoveredLocalPath ?? "",
							localFile.unavailableErrorCode,
						)
					: pathExists === false
			...
			const fileSizeBytes =
				liveFileSizeBytes ?? readRecordNumber(item.metadata, ["fileSizeBytes", "size"]);
			...
			const fileName =
				item.fileName ??
				cachedConversationAsset?.name ??
				(discoveredLocalPath ? path.basename(discoveredLocalPath) : null);
			const mimeType =
				item.mimeType ??
				cachedConversationAsset?.mimeType ??
				(fileName ? inferArchiveAssetMimeType(fileName) : null);
			...
			return {
				...
				metadata: {
					...
					...(unavailableEvidence
						...
						: {}),
					...(discoveredLocalPath
						? { localPath: discoveredLocalPath, path: discoveredLocalPath }
						: {}),
					...(cachedConversationAsset?.remoteUrl
						? { remoteUrl: cachedConversationAsset.remoteUrl }
						: {}),
					...(fileName ? { fileName } : {}),
					...(mimeType ? { mimeType } : {}),
...
function clearRefreshOwnedUnavailableMetadata(
	metadata: Record<string, unknown>,
...
function buildGeneratedArtifactUnavailableEvidence(
	item: RunArchiveItem,
...
function buildMissingLocalFileEvidence(
	item: RunArchiveItem,
	localPath: string,
): Record<string, unknown> | null {
	if (item.kind !== "generated_artifact" && item.kind !== "upload") return null;
	return {
		unavailableReason: item.mediaGenerationId
			? "media-artifact-local-file-missing"
			: "local-file-missing",
...
function fileMetadataChanged(previous: RunArchiveItem | undefined, next: RunArchiveItem): boolean {
	...
	return (
		...
		JSON.stringify(previous.metadata.materialization ?? null) !==
			JSON.stringify(next.metadata.materialization ?? null) ||
...
async function findExistingMaterializedArchiveFile(item: RunArchiveItem): Promise<string | null> {
	...
	const dir = path.join(
		getRunArchiveDir(),
		"materialized",
		sanitizeArchiveMaterializedPathSegment(item.id),
	);
	...
	if (expectedFileName) {
		const match = files.find(
			(file) => normalizeArchiveComparableString(path.basename(file)) === expectedFileName,
		);
...
async function inspectArchiveLocalFile(localPath: string | null): Promise<{
	...
	try {
		...
		const pathExists = await fileExists(localPath);
		const fileSizeBytes = pathExists ? await readFileSize(localPath) : null;
		if (!pathExists) fileChecksumCache.delete(localPath);
		return { checksumSha256, pathExists, fileSizeBytes, unavailableErrorCode: null };
	} catch (error) {
		if (isMissingFileError(error)) {
			fileChecksumCache.delete(localPath);
			return {
				checksumSha256: null,
				pathExists: false,
				fileSizeBytes: null,
		...
		if (!unavailableErrorCode) throw error;
		fileChecksumCache.delete(localPath);
		return {
			checksumSha256: null,
			pathExists: false,
...
function readUnavailableArchiveAssetErrorCode(error: unknown): string | null {
	...
	return ["ENODEV", "ESTALE", "EIO", "EACCES", "EPERM"].includes(error.code)
		? error.code
		: null;
}
...
async function calculateFileSha256(localPath: string | null): Promise<string | null> {
	...
	const cached = fileChecksumCache.get(localPath);
	if (cached && cached.mtimeMs === stats.mtimeMs && cached.size === stats.size) {
		return cached.checksumSha256;
	}
	const readKey = `${localPath}\0${stats.mtimeMs}\0${stats.size}`;
	const existingRead = fileChecksumReads.get(readKey);
	if (existingRead) return existingRead;
	const read = fs
		.readFile(localPath)
		.then((content) => createHash("sha256").update(content).digest("hex"))
		.then((checksumSha256) => {
			fileChecksumCache.set(localPath, {
				mtimeMs: stats.mtimeMs,
				size: stats.size,
				checksumSha256,
			});
			if (fileChecksumCache.size > 2_048) {
				const oldestPath = fileChecksumCache.keys().next().value;
				if (oldestPath) fileChecksumCache.delete(oldestPath);
			}
			return checksumSha256;
		})
		.finally(() => {
			fileChecksumReads.delete(readKey);
		});
	fileChecksumReads.set(readKey, read);
	return read;
}
...
function isArtifactOutput(
	item: ExecutionResponseOutputItem,
): item is Extract<ExecutionResponseOutputItem, { type: "artifact" }> {
