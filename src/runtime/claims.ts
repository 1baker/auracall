...
import {
	createExecutionRunQueueProjection,
	type ExecutionRunQueueProjection,
} from "./projection.js";
import {
...
export type ExecutionRunClaimCandidateStatus =
	| "eligible"
	| "blocked-affinity"
	| "stale-runner"
	| "not-ready";
...
export interface EvaluateExecutionRunLocalClaimInput
	extends Omit<EvaluateStoredExecutionRunLocalClaimInput, "runId"> {
...
export interface SelectStoredExecutionRunLocalClaimInput
	extends EvaluateStoredExecutionRunLocalClaimInput {}
...
export async function evaluateStoredExecutionRunLocalClaim(
	...
	return evaluateExecutionRunLocalClaim({
		inspection,
...
export function evaluateExecutionRunLocalClaim(
	input: EvaluateExecutionRunLocalClaimInput,
): ExecutionRunLocalClaimResult {
	const queue = createExecutionRunQueueProjection(input.inspection, {
		affinity: input.affinity ?? null,
	});
	if (!input.runner) {
		return {
			runId: input.inspection.record.runId,
			runnerId: input.runnerId,
			hostId: null,
	...
	return {
		...
		hostId: input.runner.hostId,
		status: classifyExecutionRunLocalClaimStatus(
			queue,
			input.runner,
			projection,
			input.now ?? null,
		...
		selected: false,
		reason: createExecutionRunLocalClaimReason(queue, input.runner, projection, input.now ?? null),
		queueState: projection.queueState,
		claimState: projection.claimState,
		affinityStatus: projection.affinity.status,
		affinityReason: projection.affinity.reason,
...
export function selectExecutionRunLocalClaim(
	...
	const evaluated = evaluateExecutionRunLocalClaim(input);
	return {
		...evaluated,
		selected: evaluated.status === "eligible",
	};
