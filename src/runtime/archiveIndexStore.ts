...
type CachedRunArchiveIndex = {
	mtimeMs: number;
	size: number;
	record: RunArchiveIndexRecord;
};

const indexCache = new Map<string, CachedRunArchiveIndex>();
const indexReadsInFlight = new Map<string, Promise<RunArchiveIndexRecord | null>>();
...
export async function readRunArchiveIndex(): Promise<RunArchiveIndexRecord | null> {
	...
	const existingRead = indexReadsInFlight.get(indexPath);
	if (existingRead) return existingRead;
	const read = (async () => {
		try {
			const raw = await fs.readFile(indexPath, "utf8");
			const record = normalizeIndexRecord(JSON.parse(raw));
			const after = await fs.stat(indexPath);
			indexCache.set(indexPath, { mtimeMs: after.mtimeMs, size: after.size, record });
			return record;
		} catch (error) {
			if (isMissingFileError(error)) {
				indexCache.delete(indexPath);
			...
			indexReadsInFlight.delete(indexPath);
		}
	})();
	indexReadsInFlight.set(indexPath, read);
	return read;
}
...
export async function writeRunArchiveIndex(
	...
	indexCache.set(indexPath, { mtimeMs: stats.mtimeMs, size: stats.size, record });
	return record;
}
