...
import type { ExecutionRunRecordBundle, ExecutionRunServiceId, ExecutionRunStep } from "./types.js";
...
export function createExecutionResponsesService(
	...
	return {
		...
		async cancelResponse(responseId, note = null) {
			const result = await host.cancelOwnedRun(responseId, note);
			if (result.cancelled && refreshArchiveIndex) {
				await refreshRunArchiveIndexBestEffort({ responseId });
			}
			return result;
		},
...
export async function createExecutionResponseForStoredRecord(
	...
	const taskRunSpecId =
		bundle.run.sourceKind === "team-run" ? (bundle.run.taskRunSpecId ?? null) : null;
...
function getStoredResponseOutput(bundle: ExecutionRunRecordBundle): ExecutionResponseOutputItem[] {
	const structured = bundle.sharedState.structuredOutputs.find(
		(entry) => entry.key === "response.output",
	);
...
function getStoredRuntimeProfile(bundle: ExecutionRunRecordBundle): string | null {
	...
	return (
		bundle.steps.find((step) => typeof step.runtimeProfileId === "string")?.runtimeProfileId ?? null
	);
}
...
export function createExecutionRequestFromRecord(
	record: ExecutionRunStoredRecord,
): ExecutionRequest {
	...
	const input =
		typeof requestInput === "string" || Array.isArray(requestInput)
			? requestInput
			: requestInput === ""
				? ""
				: "";
