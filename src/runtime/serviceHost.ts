...
import { createExecutionRuntimeControl, inspectExecutionRunStoredRecord } from "./control.js";
...
import { createExecutionRunEvent, createExecutionRunnerRecord } from "./model.js";
...
function shouldRequireOperatorApprovalForLocalAction(
...
export type ExecutionServiceHostLocalClaimSummary = {
	...
	statusByRunId: Record<string, ExecutionRunLocalClaimResult["status"]>;
...
export interface DrainStoredExecutionRunsUntilIdleOptions
	extends DrainStoredExecutionRunsOnceOptions {
	maxPasses?: number;
}

export interface DrainStoredExecutionRunsUntilIdleResult
	extends DrainStoredExecutionRunsOnceResult {
	iterations: number;
}

export interface QueuedDrainStoredExecutionRunsUntilIdleOptions
	extends DrainStoredExecutionRunsUntilIdleOptions {
	onStart?: () => void;
}
...
export interface ExecutionServiceHost {
	...
	): Promise<DrainStoredExecutionRunsOnceResult>;
	drainRunsUntilIdle(
	...
	summarizeRecoveryState(
		options?: Omit<DrainStoredExecutionRunsOnceOptions, "maxRuns">,
	): Promise<ExecutionServiceHostRecoverySummary>;
	summarizeLocalClaimState(
		options?: Omit<DrainStoredExecutionRunsOnceOptions, "maxRuns">,
	): Promise<ExecutionServiceHostLocalClaimSummary>;
	...
	controlRun(
		input: ExecutionServiceHostRunControlInput,
	): Promise<ExecutionServiceHostRunControlResult>;
	cancelOwnedRun(
		runId: string,
		note?: string | null,
...
export function createExecutionServiceHost(
	deps: ExecutionServiceHostDeps = {},
	...
	return {
		...
		async summarizeRecoveryState(
			...
			for (const candidate of await listCandidateRuns(control, options)) {
				...
				let repair: Awaited<
					ReturnType<typeof evaluateStoredExecutionRunRepairClassification>
				> | null = null;
			...
			summary.metrics = {
				...
				actionableCount:
					summary.reclaimableRunIds.length + summary.recoverableStrandedRunIds.length,
				nonExecutableCount:
		...
		async summarizeLocalClaimState(
			...
			for (const candidate of await listCandidateRuns(control, options)) {
				const inspection = inspectExecutionRunStoredRecord(candidate);
		...
		async readRecoveryDetail(runId: string) {
			...
			return {
				...
				handoffTransferSummary: readExecutionRunHandoffTransferSummaryForRecoveryDetail(
					inspection.record,
				),
				hostState:
		...
		async claimLocalRunWithSchedulerAuthority(input: ExecutionServiceHostSchedulerControlInput) {
			...
			if (!claimFutureMutation || selectedRunnerId !== localRunnerId) {
				const reason =
					selectedRunnerId && selectedRunnerId !== localRunnerId
				...
				return createConflictSchedulerClaimResult({
					...
					reason:
						error instanceof Error
							? error.message
							: `run ${input.runId} changed before scheduler claim persisted`,
				});
			}
			...
			return {
				...
				schedulerId: input.schedulerId,
				status:
					claimFutureMutation === "scheduler-reassign-expired-lease" ? "reassigned" : "claimed",
				claimed: true,
		...
		async cancelOwnedRun(runId: string, note: string | null = null) {
			...
			try {
				...
				if (
					latest &&
					latest.bundle.run.status === "cancelled" &&
					!getActiveExecutionRunLease(latest)
				) {
					return {
						action: "cancel-run",
						runId,
		...
		async drainRun(runId: string) {
			...
			if (entry.result === "executed") {
				await persistOperatorDrainEvent(
					...
					now(),
					"executed",
					"run executed through targeted host drain",
					null,
				);
				return {
					action: "drain-run",
		...
		async resolveLocalActionRequest(
			...
			const existingRequest = record.bundle.localActionRequests.find(
			...
			const ownerStep =
				record.bundle.steps.find((step) => step.id === existingRequest.ownerStepId) ?? null;
			const ownerStepRequests = updatedRequests.filter(
				(candidate) => candidate.ownerStepId === existingRequest.ownerStepId,
			);
			...
			await control.persistRun({
				...
				bundle: {
					...
					sharedState: {
						...
						structuredOutputs: [
							...record.bundle.sharedState.structuredOutputs.filter(
								(entry) => entry.key !== outcomeSummaryKey,
							),
			...
			for (const candidate of candidates) {
				...
				if (executedCount >= maxRuns) {
					drained.push({
						...
						result: "skipped",
						reason:
							inspection.dispatchPlan.nextRunnableStepId ||
							candidate.kind === "recoverable-stranded"
								? "limit-reached"
		...
		async drainRunsUntilIdle(options: DrainStoredExecutionRunsUntilIdleOptions = {}) {
			...
			return {
				...
				expiredLeaseRunIds,
				executedRunIds,
				drained: drained.filter(
					(entry): entry is DrainedStoredExecutionRunResult => entry !== null,
				),
				iterations,
			};
		},
		...
		async drainRunsUntilIdleQueued(options: QueuedDrainStoredExecutionRunsUntilIdleOptions = {}) {
			...
			const nextDrain = drainQueue
				.catch(() => null)
				.then(() => {
					onStart?.();
					return this.drainRunsUntilIdle(drainOptions);
				});
			drainQueue = nextDrain.then(
				(result) => result,
				() => null,
			);
			return nextDrain;
...
async function evaluateAndRepairStaleHeartbeatLease(input: {
	...
	if (
		isTerminalExecutionRunStatus(inspection.record.bundle.run.status) &&
		activeLease.expiresAt <= input.repairAt
	) {
	...
	if (!repair || repair.posture !== "locally-reclaimable") {
		return {
			action: {
				...
				repaired: false,
				reason:
					repair?.reason ?? "stale-heartbeat lease does not have a reclaimable repair posture",
				leaseHealthStatus: leaseHealth.status,
				repairPosture: repair?.posture ?? null,
				reconciliationReason: repair?.reconciliation.reason ?? leaseHealth.reason ?? null,
			},
...
function isTerminalExecutionRunStatus(
	status: ExecutionRunStoredRecord["bundle"]["run"]["status"],
): boolean {
...
function createTerminalCancelRaceResult(
	record: ExecutionRunStoredRecord,
): ExecutionServiceHostCancelActionResult {
	if (record.bundle.run.status === "cancelled") {
		return {
...
function readExecutionRunOrchestrationTimelineSummaryForRecoveryDetail(
	...
	const relevantEvents = record.bundle.sharedState.history.filter((event) => {
		...
		return (
			...
			(typeof event.payload.source === "string" || typeof event.payload.action === "string")
		);
	});
	...
	return {
		...
		items: relevantEvents.slice(-10).map((event) => ({
			...
			handoffId:
				isRecord(event.payload) && typeof event.payload.handoffId === "string"
...
function readExecutionRunHandoffTransferSummaryForRecoveryDetail(
	...
	const storedSummary = readStoredConsumedTaskTransferSummaryForRecoveryDetail(
		record,
		selectedStep.id,
	);
	if (storedSummary) {
...
function readStoredConsumedTaskTransferSummaryForRecoveryDetail(
	...
	const items = entry.value.items.filter(isRecord).map((item) => ({
...
function classifyHostDrainCandidate(
	...
	dispatchPlan: NonNullable<
		Awaited<ReturnType<ExecutionRuntimeControlContract["inspectRun"]>>
	>["dispatchPlan"],
	now: () => string,
...
function compareHostDrainCandidatePriority(
	left: HostDrainCandidateKind,
	right: HostDrainCandidateKind,
): number {
...
function createActionableExecutionPlan(
	candidates: HostDrainCandidateInspection[],
	maxRuns: number,
): Set<string> {
	...
	const recoverableStranded = candidates.filter(
		(candidate) => candidate.kind === "recoverable-stranded",
	);
...
function projectRunnerLifecycleState(
	runner: ExecutionRunnerRecord,
): ExecutionServiceHostRunnerLifecycleState {
	return {
		id: runner.id,
...
function activeLeasesMatch(
	left: ExecutionRunLease | null,
	right: ExecutionRunLease | null,
): boolean {
...
async function listCandidateRuns(
	...
	return records.sort((left, right) =>
		left.bundle.run.createdAt.localeCompare(right.bundle.run.createdAt),
	);
}
