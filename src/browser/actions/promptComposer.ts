...
export async function submitPrompt(
	...
	await waitForComposerReadyToSubmit(runtime, Math.max(8_000, deps.inputTimeoutMs ?? 0), deps.requireSendButton === true);
	await deps.beforeSend?.();
	const clicked = await attemptSendButton(runtime, logger, deps?.attachmentNames, deps.requireSendButton === true);
	if (!clicked) {
...
async function waitForComposerReadyToSubmit(
	...
	if (requireSendButton) {
		throw new BrowserAutomationError("ChatGPT composer did not become ready before submission; refusing to dispatch.", {
			stage: "submit-prompt", code: "composer-not-ready", timeoutMs,
		});
	}
}
...
async function verifyPromptCommitted(
	...
	const script = `(() => {
	    ...
	    const usersWithIds = Array.from(document.querySelectorAll('[data-message-author-role="user"][data-message-id]'));
	    const latestUser = usersWithIds[usersWithIds.length - 1];
	    const latestUserId = latestUser?.getAttribute('data-message-id') ?? '';
	    const latestMatchingUserId = latestUserId && usersWithIds.filter((node) => node.getAttribute('data-message-id') === latestUserId).length === 1 && normalize(readCommittedTurnText(latestUser)) === normalizedPrompt ? latestUserId : null;
	...
	while (Date.now() < deadline) {
		...
		if (baselineUserId && info?.latestMatchingUserId && info.latestMatchingUserId !== baselineUserId && info.composerCleared && info.inConversation) {
