...
export async function submitPrompt(
	...
	let observedInitialText = targetUserTextRaw || editorUserTextRaw || fallbackValueRaw;
	if (!observedInitialText.trim()) {
		// A slow ProseMirror render may expose the inserted text after Input.insertText returns.
		// Give that original insertion a bounded readback before attempting a fallback write.
		const delayedReadback = await waitForPromptInComposer(
			runtime,
			prompt,
		...
		if (snapshotContainsPrompt(delayedReadback, prompt)) {
			observedInitialText = prompt;
		}
	}
	if (!observedInitialText.trim()) {
		...
		!composerContainsPrompt(observedInitialText, prompt) &&
