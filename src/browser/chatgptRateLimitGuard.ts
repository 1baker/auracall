...
export const CHATGPT_RATE_LIMIT_ESCALATION_WINDOW_MS = 24 * 60 * 60_000;
export const CHATGPT_RATE_LIMIT_MAX_COOLDOWN_MS = 6 * 60 * 60_000;
...
export type ChatgptRateLimitGuardState = {
	...
	recentRateLimitDetectionAts?: number[];
...
export function resolveChatgptRateLimitProfileName(
	options: {
...
export function resolveChatgptRateLimitGuardPath(
	options: {
		profileName?: string | null;
		managedProfileDir?: string | null;
		managedProfileRoot?: string | null;
...
export async function readChatgptRateLimitGuardState(
	options: {
		profileName?: string | null;
		managedProfileDir?: string | null;
		managedProfileRoot?: string | null;
	...
	try {
		...
		return {
			...
			recentMutationAts: Array.isArray(parsed.recentMutationAts)
				? parsed.recentMutationAts.filter(
			...
			recentMutations: Array.isArray(parsed.recentMutations)
				? parsed.recentMutations
						.filter(
							(value): value is ChatgptMutationRecord =>
				...
				: undefined,
			recentRateLimitDetectionAts: Array.isArray(parsed.recentRateLimitDetectionAts)
				? parsed.recentRateLimitDetectionAts.filter(
						(value): value is number => typeof value === "number" && Number.isFinite(value),
					)
				: undefined,
			cooldownUntil: typeof parsed.cooldownUntil === "number" ? parsed.cooldownUntil : undefined,
			cooldownDetectedAt:
				typeof parsed.cooldownDetectedAt === "number" ? parsed.cooldownDetectedAt : undefined,
...
export function resolveChatgptRateLimitCooldownMs(
	state:
		| Pick<
				ChatgptRateLimitGuardState,
				| "cooldownDetectedAt"
				| "cooldownUntil"
				| "recentMutations"
				| "recentMutationAts"
				| "recentRateLimitDetectionAts"
		  >
		| null
		| undefined,
	now = Date.now(),
	...
	const escalationWindowMs = options.escalationWindowMs ?? CHATGPT_RATE_LIMIT_ESCALATION_WINDOW_MS;
	const maxCooldownMs = options.maxCooldownMs ?? CHATGPT_RATE_LIMIT_MAX_COOLDOWN_MS;
	const retryAfterMs =
	...
	const recentDetections = pruneChatgptRateLimitDetectionHistory(
		state?.recentRateLimitDetectionAts,
		now,
		escalationWindowMs,
	);
	if (recentDetections.length > 0) {
		const escalatedCooldownMs = Math.min(
			maxCooldownMs,
			baseCooldownMs * 3 ** recentDetections.length,
		);
		return Math.max(cooldownMs, escalatedCooldownMs);
	}
	const detectedAt =
...
export function pruneChatgptRateLimitDetectionHistory(
	history: readonly number[] | null | undefined,
	now = Date.now(),
	windowMs = CHATGPT_RATE_LIMIT_ESCALATION_WINDOW_MS,
): number[] {
	return (history ?? [])
		.filter((value) => Number.isFinite(value) && value > now - windowMs && value <= now)
		.sort((left, right) => left - right);
}

export function appendChatgptRateLimitDetection(
	history: readonly number[] | null | undefined,
	now = Date.now(),
	windowMs = CHATGPT_RATE_LIMIT_ESCALATION_WINDOW_MS,
): number[] {
	return [...pruneChatgptRateLimitDetectionHistory(history, now, windowMs), now];
}
	...
	return history
		.filter(
			(value): value is number =>
				typeof value === "number" && Number.isFinite(value) && value > cutoff,
		)
		.sort((left, right) => left - right);
}
...
export function getChatgptMutationQuietJitterMs(
	...
	const key = `${String(action ?? "")
		.trim()
		.toLowerCase()}#${Math.max(0, Math.trunc(recentCount))}`;
...
export function normalizeChatgptMutationRecords(
	...
	if (Array.isArray(state?.recentMutations) && state.recentMutations.length > 0) {
		return state.recentMutations
			.filter(
				(value): value is ChatgptMutationRecord => Boolean(value) && typeof value === "object",
			)
			.map((value) => ({
				at: typeof value.at === "number" && Number.isFinite(value.at) ? value.at : 0,
				action: typeof value.action === "string" ? value.action : undefined,
...
export function pruneChatgptMutationRecords(
	history: readonly ChatgptMutationRecord[] | readonly number[] | null | undefined,
	now = Date.now(),
	windowMs = CHATGPT_MUTATION_WINDOW_MS,
): ChatgptMutationRecord[] {
	const cutoff = now - Math.max(0, windowMs);
...
export function getChatgptMutationBudgetWaitMs(
	...
	const history = pruneChatgptMutationRecords(
		normalizeChatgptMutationRecords(state),
		now,
		windowMs,
	);
	if (history.length === 0) {
...
export function getChatgptPostCommitQuietWaitMs(
	...
	const quietScale =
		typeof options.quietScale === "number" &&
		Number.isFinite(options.quietScale) &&
		options.quietScale > 0
			? options.quietScale
			: 1;
	const history = pruneChatgptMutationRecords(
		normalizeChatgptMutationRecords(state),
	...
	const baseQuietMs =
		(typeof lastRecord.quietMs === "number"
			? lastRecord.quietMs
			: CHATGPT_POST_COMMIT_BASE_QUIET_MS) * quietScale;
