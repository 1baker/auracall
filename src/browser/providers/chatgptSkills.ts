...
import { ensureRequiredChatgptProIntelligence } from "../actions/thinkingTime.js";
			...
			await submitPrompt(
				{
					...
					beforeSend: async () => {
						this.throwIfAborted();
						await ensureRequiredChatgptProIntelligence(client.Runtime, logger);
	...
	private async withSelectedSkill(
		...
		try {
			...
			if (!proofReady.ok) {
				...
				if (typeof onSelected === "function") return await onSelected(client);
			}
		} catch (error) {
