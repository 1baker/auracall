...
- `--browser-chatgpt-mode <chat|work>`: select the ChatGPT composer mode. Omitted values resolve to `chat`. AuraCall verifies an exact visible radio using `aria-checked` or `data-state`; on a project page with no switcher, it accepts only the observed composer contract (`role="textbox"`, `contenteditable="true"`, a non-empty `New chat in ...` accessible label, and a containing form). Use `work` only for a deliberately requested Work run.
