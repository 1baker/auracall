...
- Exact ChatGPT artifact-control and download binding is provider-free:
  `pnpm exec vitest run tests/browser/chatgptArtifactControlsExact.test.ts tests/browser/chatgptArtifactDownloadBinding.test.ts tests/browser/chatgptAdapter.test.ts tests/browser/llmServiceFiles.test.ts`.
  It proves exact response-owned filename selection, ambiguity rejection, fresh
  single-download isolation, partial-download rejection, and byte-preserving
  canonicalization. It does not prove the current live ChatGPT DOM or installed
  runtime.
...
- CI Node runtime contract: `pnpm run check:ci-runtime` binds the canonical
  ...
  bypass. It also retains the maintained, exactly pinned Homebridge PTY runtime,
  the current Windows runner, and the Node 24-compatible pnpm setup action. The
  workflow retains `workflow_dispatch` so
  ...
  operating system. `pnpm run test:pty` separately proves real spawn/data,
  resize, interrupt, exit, streaming, and TUI behavior on every matrix host.
  Platform-specific fixtures gate only the exact host
...
- Durable configured-browser cancellation is provider-free:
  ...
  It proves durable cancellation aborts active step work, the configured
  executor forwards the same signal, ChatGPT response polling exits promptly,
  and late completion cannot overwrite terminal cancelled state.
...
- Gemini semantic selector execution is provider-free:
  `pnpm exec vitest run tests/config/modelSelector.test.ts tests/browser/geminiModelSelection.test.ts tests/browser/llmServiceContext.test.ts tests/runtime.configuredExecutor.test.ts tests/config/agentConfigService.test.ts tests/http.responsesServer.test.ts tests/services/registry.test.ts`.
  This proves semantic-to-current-label mapping, exact picker verification,
  attachment staging, shared prompt transport, native configured execution,
  typo rejection, and discovery readiness without launching Gemini.
...
- ChatGPT cancelled-local-action team live smoke:
  - portable local-action examples assume
    `AURACALL_REPO_ROOT="$(git rev-parse --show-toplevel)"` is set
  ...
  - current acceptance bar:
    - initial `auracall-chatgpt-tooling` run uses:
      ...
      - `--allow-local-cwd-root "$AURACALL_REPO_ROOT"`
