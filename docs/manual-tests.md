...
9. Run the bounded Gemini tooling workflow smoke when validating Gemini-bound
   ...
   ORACLE_NO_BANNER=1 NODE_NO_WARNINGS=1 DISPLAY=:0.0 \
     ...
     --structured-context-json '{"toolEnvelope":{"kind":"shell","summary":"Run one bounded deterministic node command","command":"node","args":["-e","process.stdout.write('\''AURACALL_TOOL_ACTION_OK'\'')"],"structuredPayload":{"cwd":"'"$AURACALL_REPO_ROOT"'"}},"finalToken":"AURACALL_GEMINI_TOOL_TEAM_SMOKE_OK"}' \
     --max-turns 2 \
     --allow-local-shell-command node \
     --allow-local-cwd-root "$AURACALL_REPO_ROOT" \
     --json
