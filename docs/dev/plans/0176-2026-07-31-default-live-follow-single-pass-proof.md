...
- [ ] Baseline proves the installed Plan 0175 bundles and paused/guard-clear
  default lane before mutation.
- [ ] Exactly one `run_one_pass` action is accepted for the named completion.
- [ ] All other completions and the global scheduler remain paused.
- [ ] The pass reaches a terminal bounded posture with pass count no greater
...
- [ ] Any all-failed materialization is reported `failed` and blocks the
  ...
  metrics consistent with that status.
- [ ] No new rate-limit or human-verification observation occurs; if one does,
  the proof stops immediately and remains paused/blocked.
- [ ] Plan, roadmap, runbook, and durable closeout evidence match the observed
  installed state.
