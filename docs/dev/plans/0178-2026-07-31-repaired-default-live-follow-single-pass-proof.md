...
- [ ] The pushed installed repair and paused/guard-clear baseline are proved
  before mutation.
- [ ] Exactly one `run_one_pass` is accepted for the named completion while the
  scheduler and all other completions remain paused.
- [ ] The collector advances from pass 36 to at most 37 and does not run a
  second collection.
- [ ] Same-email provider identity with omitted qualifiers no longer produces
  ...
  stops fail-closed.
- [ ] Completion-owned materialization reaches terminal before forced control
  settlement: failure blocks with `account_mirror_materialization_failed`, or
  success/skip returns idle with consistent metrics.
- [ ] No new rate-limit, CAPTCHA, verification, or provider-guard observation
  occurs; if one occurs, all lanes remain or are restored paused immediately.
- [ ] Plan, roadmap, runbook, journal, and installed-runtime closeout evidence
