...
- configured single-agent calls can use OpenAI-style model routing:
  ...
  - `modelSelector` is the stable semantic intent field, e.g.
    ...
    `chatgpt:reasoning-max`, `chatgpt:premium`, `chatgpt:legacy`,
    `grok:auto`, `grok:instant`, `grok:thinking`, `gemini:auto`,
    `gemini:instant`, or `gemini:thinking`
  ...
  - Gemini browser-backed execution resolves `gemini:auto`,
    ...
    Gemini Flash-Lite, and Gemini Pro picker rows through the native provider
    adapter; missing menus, exact rows, or selected-state verification stop
    execution before prompt insertion
  - configured Gemini prompts preserve native-adapter attachments and do not
    default to legacy private web-RPC model headers; raw `model` remains the
    explicit higher-priority compatibility escape hatch
